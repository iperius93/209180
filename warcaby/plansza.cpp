...
plansza::plansza()
{
	...
	for(x=0;x<4;x++)
	{
		ustawienie[x].prawy=&ustawienie[x+4];
		ustawienie[x+8].prawy=&ustawienie[x+4+8];
		ustawienie[x+16].prawy=&ustawienie[x+4+16];
		ustawienie[x+24].prawy=&ustawienie[x+4+24];
		ustawienie[4+x].lewy=&ustawienie[x+8];
		ustawienie[12+x].lewy=&ustawienie[x+16];
		ustawienie[20+x].lewy=&ustawienie[x+24];
		ustawienie[4+x].lewy=&ustawienie[x+4+4];
	}

	for(x=0;x<3;x++)
	{
		ustawienie[4+x].prawy=&ustawienie[4+5+x];
		ustawienie[12+x].prawy=&ustawienie[12+5+x];
		ustawienie[20+x].prawy=&ustawienie[20+5+x];

		ustawienie[1+x].lewy=&ustawienie[4+x];
		ustawienie[9+x].lewy=&ustawienie[12+x];
		ustawienie[17+x].lewy=&ustawienie[20+x];
		ustawienie[25+x].lewy=&ustawienie[28+x];
	}
	ustawienie[0].lewy=NULL;
	ustawienie[8].lewy=NULL;
	ustawienie[16].lewy=NULL;
	ustawienie[24].lewy=NULL;
	...
	ustawienie[28].prawy=NULL;
	ustawienie[29].prawy=NULL;
	ustawienie[30].prawy=NULL;
	ustawienie[31].prawy=NULL;

	ustawienie[7].prawy=NULL;
	ustawienie[15].prawy=NULL;
	ustawienie[23].prawy=NULL;
...
void plansza::Przesun(int skad, int dokad)
{
	ustawienie[dokad].typ=ustawienie[skad].typ;
	ustawienie[skad].typ=Pusty;
}

void plansza::Przesun_na_kopi(int skad, int dokad)
{
    tmp_ustawienie[dokad] = tmp_ustawienie[skad];
	tmp_ustawienie[skad] = Pusty;

}

int plansza::XYtoI(int x, int y)
{
	return (y-1)*4+(x-1)/2;
}

XY plansza::ItoXY(int i)
{
  XY temp;

  temp.x=(i)%4*2+(i)/4%2+1;
  temp.y=(i)/4+1;
  return temp;
};
...
void plansza::Ruch_Komputera(pionek CzyjRuch)
{
